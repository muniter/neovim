...
dozet:
  // "zf" and "zF" are always an operator, "zd", "zo", "zO", "zc" and "zC"
  // only in Visual mode. "zj", "zk", "z'", "z`" are motions.
  if (cap->nchar != 'f' && cap->nchar != 'F'
      && !(VIsual_active && vim_strchr("dcCoO", cap->nchar))
      && cap->nchar != 'j' && cap->nchar != 'k' && cap->nchar != '`' && cap->nchar != '\''
  ...
  // "z'{mark}", "z`{mark}" restore the view of the last used named mark, this should be a drop in
  // replacement for "'{mark}" and "`{mark}", prepare cap and send to nv_gomark()
  case '\'':
    cap->arg = true;
    FALLTHROUGH;
  case '`':
    cap->count0 = 1;  // when nv_gomark receives a count it restores the view e.g [count]'a
    cap->nchar = plain_vgetc();  // Next character is the mark name
    nv_gomark(cap);
    return;
  default:
    clearopbeep(cap->oap);
...
{
  int name;
  MarkMove flags = 0;  // flags for moving to the mark
